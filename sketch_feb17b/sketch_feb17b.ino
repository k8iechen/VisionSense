...
void loop() {
  ...
  if (stepper1.distanceToGo() == 0 && Serial.available()) {
    r=Serial.read()-'0';
    if (r ==1 || r == 4 || r == 7){
    stepper1.moveTo(stepper1.currentPosition()+((r+2)*100));
  }   
  }
  stepper1.run();

  // Turn on or off the LED as determined by the state :
  if (stepper2.distanceToGo() == 0 && Serial.available()) {
    r=Serial.read()-'0';
    if (r ==2 || r == 5 || r == 8){
      stepper2.moveTo(stepper2.currentPosition()+((r+1)*100));
  } 
  }
  stepper2.run();
  ...
  // Turn on or off the LED as determined by the state :
  if (stepper3.distanceToGo() == 0 && Serial.available()) {
    r=Serial.read()-'0';
    if (r ==3 || r == 6 || r == 9){
    
    stepper3.moveTo(stepper3.currentPosition()+(r*100));
